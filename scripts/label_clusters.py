...
class VibeVector(BaseModel):
    ...
    frequency: float = Field(
        ge=0.0, le=1.0, description="0=everyday, 1=special occasion"
    )
    ...
    longevity: float = Field(
        ge=0.0, le=1.0, description="0=light/short, 1=heavy/long-lasting"
    )
...
def label_clusters_with_llm(
    df, embeddings, kmeans, n_clusters, model_name="Qwen/Qwen3-Embedding-0.6B", n_reps=5
):
    """
...
def get_fragrance_vibe(
    fragrance_idx: int, df: pd.DataFrame, cluster_vibes: dict
) -> np.ndarray:
