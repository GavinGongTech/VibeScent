...
for i in range(len(outfit_names)):
    for j in range(i + 1, len(outfit_names)):
        ...
        if overlap:
            print(
                f"⚠ Overlap between '{outfit_names[i]}' and '{outfit_names[j]}': clusters {overlap}"
            )
            all_unique = True  # overlap is okay if not complete
        if all_top_clusters[i] == all_top_clusters[j]:
            print(
                f"✗ IDENTICAL results for '{outfit_names[i]}' and '{outfit_names[j]}' — scoring needs work"
            )
            all_unique = False
...
for i, dim in enumerate(VIBE_DIMENSIONS):
    vals = all_vecs[:, i]
    print(
        f"  {dim:12s}: min={vals.min():.2f}  max={vals.max():.2f}  "
...
for i, dim in enumerate(VIBE_DIMENSIONS):
    if all_vecs[:, i].std() < 0.05:
        print(
            f"⚠ '{dim}' has very low variance — clusters aren't differentiating on this dimension"
        )
